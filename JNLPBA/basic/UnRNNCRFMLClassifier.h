/*
 * UnRNNCRFMLClassifier.h
 *
 *  Created on: 2016-11-11
 *      Author: chen_lyu
 */

#ifndef UNRNNCRFMLCLASSIFIER_H_
#define UNRNNCRFMLCLASSIFIER_H_
...
class UnRNNCRFMLClassifier {
public:
  UnRNNCRFMLClassifier() {
    _dropOut = 0.5;
  }
  ~UnRNNCRFMLClassifier() {
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, const NRMat<dtype>& charEmb, int charcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int charhiddensize,
    ...
    _tanh_project.initial(_hiddensize, _rnnhiddensize, true, 55, 0);
...
//add right-left content
      ...
      _tanh_project.ComputeForwardScore(left_project, project);
...
// change to CRF loss
      ...
      _tanh_project.ComputeBackwardLoss(left_project, project, projectLoss, left_projectLoss);
